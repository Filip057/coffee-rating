...
class TestReviewList:
    ...
    def test_list_reviews_filter_by_coffeebean(self, api_client, review, coffeebean):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data['results']) == 1
        assert str(response.data['results'][0]['coffeebean']) == str(coffeebean.id)
...
class TestReviewCreate:
    ...
    def test_create_review(self, authenticated_client, another_coffeebean, user):
        """Create a new review."""
        url = reverse('reviews:review-list')
        data = {
            'coffeebean': str(another_coffeebean.id),
            'rating': 5,
            'aroma_score': 4,
            'flavor_score': 5,
    ...
    def test_create_review_auto_creates_library_entry(self, authenticated_client, another_coffeebean, user):
    ...
    def test_create_review_updates_bean_rating(self, authenticated_client, another_coffeebean):
    ...
    def test_create_review_duplicate_forbidden(self, authenticated_client, coffeebean, review):
    ...
    def test_create_review_unauthenticated(self, api_client, coffeebean):
    ...
    def test_create_review_invalid_rating(self, authenticated_client, another_coffeebean):
...
class TestReviewUpdate:
    ...
    def test_update_own_review(self, authenticated_client, review):
    ...
    def test_update_others_review_forbidden(self, other_client, review):
        """Cannot update another user's review."""
        url = reverse('reviews:review-detail', args=[review.id])
        data = {'rating': 1}
        response = other_client.patch(url, data, format='json')
...
class TestReviewDelete:
    ...
    def test_delete_own_review(self, authenticated_client, review, coffeebean):
    ...
    def test_delete_others_review_forbidden(self, other_client, review):
...
class TestMyReviews:
    ...
    def test_my_reviews(self, authenticated_client, review, other_review):
...
class TestUserLibrary:
    ...
    def test_get_library(self, authenticated_client, library_entry):
    ...
    def test_get_library_excludes_archived(self, authenticated_client, library_entry, archived_library_entry):
    ...
    def test_get_archived_library(self, authenticated_client, library_entry, archived_library_entry):
        """Get archived library entries."""
        url = reverse('reviews:user-library')
        response = authenticated_client.get(url, {'archived': 'true'})
...
class TestAddToLibrary:
    ...
    def test_add_to_library(self, authenticated_client, another_coffeebean, user):
        """Add coffee bean to library."""
        url = reverse('reviews:add-to-library')
        data = {'coffeebean_id': str(another_coffeebean.id)}
        response = authenticated_client.post(url, data, format='json')
    ...
    def test_add_to_library_duplicate(self, authenticated_client, library_entry, coffeebean):
        """Adding duplicate returns existing entry."""
        url = reverse('reviews:add-to-library')
        data = {'coffeebean_id': str(coffeebean.id)}
        response = authenticated_client.post(url, data, format='json')

        # Returns 200 OK for existing entry (not 201 Created)
        assert response.status_code == status.HTTP_200_OK

    def test_add_nonexistent_bean(self, authenticated_client):
...
class TestArchiveLibraryEntry:
    ...
    def test_archive_entry(self, authenticated_client, library_entry):
        """Archive a library entry."""
        url = reverse('reviews:archive-library-entry', args=[library_entry.id])
        data = {'is_archived': True}
        response = authenticated_client.patch(url, data, format='json')

        assert response.status_code == status.HTTP_200_OK
        library_entry.refresh_from_db()
        assert library_entry.is_archived is True

    def test_unarchive_entry(self, authenticated_client, archived_library_entry):
        """Unarchive a library entry."""
        url = reverse('reviews:archive-library-entry', args=[archived_library_entry.id])
        data = {'is_archived': False}
        response = authenticated_client.patch(url, data, format='json')

        assert response.status_code == status.HTTP_200_OK
        archived_library_entry.refresh_from_db()
        assert archived_library_entry.is_archived is False

    def test_archive_others_entry_forbidden(self, other_client, library_entry):
...
class TestRemoveFromLibrary:
    ...
    def test_remove_from_library(self, authenticated_client, library_entry):
        """Remove entry from library."""
        url = reverse('reviews:remove-from-library', args=[library_entry.id])
        response = authenticated_client.delete(url)

        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not UserLibraryEntry.objects.filter(id=library_entry.id).exists()

    def test_remove_others_entry_forbidden(self, other_client, library_entry):
...
class TestCreateTag:
    ...
    def test_create_duplicate_tag(self, authenticated_client, tag_fruity):
...
class TestBeanReviewSummary:
    ...
    def test_bean_review_summary_url_resolves(self, coffeebean):
        """Bean review summary URL can be resolved."""
        url = reverse('reviews:bean-review-summary', args=[coffeebean.id])
        assert '/api/reviews/bean/' in url
        assert str(coffeebean.id) in url
...
class TestReviewModel:
    ...
    def test_unique_author_bean(self, user, coffeebean, review):
...
class TestUserLibraryEntryModel:
    ...
    def test_entry_str(self, library_entry):
        """Test string representation."""
        assert library_entry.user.get_display_name() in str(library_entry)
        assert library_entry.coffeebean.name in str(library_entry)

    def test_ensure_entry_creates_new(self, user, another_coffeebean):
        ...
        assert entry.coffeebean == another_coffeebean

    def test_ensure_entry_returns_existing(self, user, coffeebean, library_entry):
        ...
        assert entry.id == library_entry.id
